...
do
    ...
    docker buildx build -f ${target}/public/publish/docker/linux-x64/${f}/Dockerfile --platform="linux/x86_64"  --force-rm -t "${image}-x64:latest" -t "${image}-x64:v1.1.3" . --push
    cd ../../../../../

    cd public/publish/docker/linux-arm64/${f}
    docker buildx build -f ${target}/public/publish/docker/linux-arm64/${f}/Dockerfile --platform="linux/arm64"  --force-rm -t "${image}-arm64:latest" -t "${image}-arm64:v1.1.3" . --push
    cd ../../../../../

    cd public/publish/docker/linux-arm/${f}
    docker buildx build -f ${target}/public/publish/docker/linux-arm/${f}/Dockerfile --platform="linux/arm/v7"  --force-rm -t "${image}-arm:latest" -t "${image}-arm:v1.1.3" . --push
    cd ../../../../../
